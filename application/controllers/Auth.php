...
class Auth extends CI_Controller
{
        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                ...
                $this->session->set_flashdata(
                    'message',
                    '<div class="alert
            alert-danger" role="alert">This email has not as actived!</div>'
                );
                redirect('auth');
            }
        } else {
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message', '<div class="alert
            alert-success" role="alert">Congratulation! 
            your account has been created. please active your acount!</div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'irpaasincan@gmail.com',
            'smtp_pass' => 'irparosy02',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n"
        ...
        $this->email->from('irpaasincan@gmail.com', 'irpa rosy');
        ...
        if ($type == 'verify') {
            ...
            $this->email->message('Click this link to verify you account : <a href="' . base_url() .
                'auth/verify?email=' . $this->input->post('email') . '&token=' .  urlencode($token) . '">Active</a>');
        } else if ($type == 'forgot') {
    ...
    public function forgotPassword()
    {
        ...
        alert-success" role="alert">Please check your email to reset your password!</div>');
                ...
                $this->session->set_flashdata('message', '<div class="alert
        alert-danger" role="alert">Email is not registered or activeted!</div>');
                redirect('auth/forgotpassword');
            }
        }
    }
